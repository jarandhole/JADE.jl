...
function JADEsddp(d::JADEData, optimizer = nothing)
    ...
    #------------------------------------------------------------------------
    graph = SDDP.LinearGraph(number_of_wks + 1) # Investment version: adding one extra node for investment node
    ...
    if d.rundata.weekly_discounting && d.rundata.discount != 0
        ...
        for (index, obj) in graph.nodes
            if index != 0 && index != 1 # Investment version: No discounting from investment stage to first "week" (second node)
    ...
    sddpm = SDDP.PolicyGraph(
        ...
        JuMP.@variable(
            ...
            (r in s.INVESTABLES ? d.investables[r].max_investment : d.reservoirs[r].capacity[timenow] / scale_factor), # unbounded for investable reservoirs, need to be bounded by constraints
            SDDP.State,
            ...
            if d.rundata.steady_state 
                SDDP.@stageobjective(
                    ...
                    sum(
                        ...
                        investment_decision[i] for i in s.INVESTABLES
                    )
                )
            else
                SDDP.@stageobjective(
        ...
        # Investment version: the rest of the model is defined for stages after investment stage
            ...
            JuMP.@variables(
                md,
                begin
                    # Investment version: adding explisit wind_generation decision variable (TODO: find name, if just new capacity or all (not residual demand))
                    new_wind_gen[s.WINDS, s.BLOCKS] >= 0
                    # Investment version: adding explisit solar_generation decision variable (TODO: find name, if just new capacity or all (not residual demand))
                    new_solar_gen[s.SOLARS, s.BLOCKS] >= 0
            ...
            JuMP.@variables(
                ...
                begin
                    ...
                    fuel_costs[f in s.THERMALS, bl in s.BLOCKS] in JuMP.MOI.Parameter(1.0)
                end
            )
            ...
            JuMP.@constraints(
                ...
                begin
                    ...
                    transflow[(n, m), bl] <=
                    d.transmission[(n, m)].poscapacity + (str2sym(string(n)*"_TO_"*string(m)) in s.INVESTABLES ? invested_capacity[str2sym(string(n)*"_TO_"*string(m))].in : 0) - # Investment version: adding invested capacity to transmission capacity
                    d.transmission[(n, m)].posoutage[timenow][bl]
                    transLower[(n, m) in s.TRANS_ARCS, bl in s.BLOCKS],
                    -d.transmission[(n, m)].negcapacity - (str2sym(string(n)*"_TO_"*string(m)) in s.INVESTABLES ? invested_capacity[str2sym(string(n)*"_TO_"*string(m))].in : 0) + # Investment version: adding invested capacity to transmission capacity, same for both ways so far
            ...
            for scenario in 1:length(d.inflow_mat[1][s.CATCHMENTS_WITH_INFLOW[1]])
                ...
                if stage == 2 && d.rundata.first_week_known # Investment version: changes stage from 1 to 2
