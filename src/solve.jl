...
function optimize_policy!(
    ...
    @info("Using the customized NVE-JADE with investments")
    ...
    if solveoptions.iterations > 0
        ...
        if d.rundata.steady_state && !solveoptions.reset_starting_levels
            solveresults = SDDP.train(
                ...
                sampling_scheme = SDDP.Historical(sample_paths; terminate_on_dummy_leaf = true), # terminating on dummy leaf, document whats happening
